...
function addLoan(data) {
  try {
    ...
    const loan = {
      ...
      name: data.name,
      principal: principal,
      balance: data.balance ? parseAmount(data.balance) : principal,
      interest_rate: data.interest_rate || 0,
      repayment_type: data.repayment_type || '원금+이자',
      monthly_payment: data.monthly_payment ? parseAmount(data.monthly_payment) : 0,
